...
import com.example.Buoi2.config.VNPayConfig;
...
public class OrderService {
    ...
    public Order createOrder(String customerName, String shippingAddress, String phoneNumber, String email, String notes, List<CartItem> cartItems, String discountCode) {
        ...
        order.setOrderDate(new Date()); // Set ngày đặt hàng hiện tại
        return orderRepository.save(order);
    }
    @Transactional
    public String checkOutWithPayOnline(String customerName, String shippingAddress, String phoneNumber, String email, String notes, List<CartItem> cartItems, String discountCode, String urlReturn) {
        if (cartItems == null) {
            throw new IllegalArgumentException("Cart items must not be null");
        }
        Order order = new Order();
        order.setCustomerName(customerName);
        order.setShippingAddress(shippingAddress);
...
//       Value Amount
        vnp_Params.put("vnp_Amount", String.valueOf((int)orders.getTotalAmount()*100));
        vnp_Params.put("vnp_CurrCode", "VND");

        vnp_Params.put("vnp_TxnRef", vnp_TxnRef);
        // customer information
        vnp_Params.put("vnp_OrderInfo", orders.getCustomerName() + " " + orders.getShippingAddress());
...
//---------------------------------------
   ...
   /* public List<Map<String, Object>> getRevenueStatistics() {
    ...
    public Order updateOrder(Order updatedOrder) {
        ...
        return null; // Hoặc xử lý nếu không tìm thấy đơn hàng
    }
}
